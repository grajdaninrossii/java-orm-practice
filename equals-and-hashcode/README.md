...
Во-вторых, тебе писать не нужно, попроси своего СТО, пускай он купит лицензию и будешь генерить эти методы в два клика.  
Но я скажу вам что:  
...
- lombok для сущностей вреден. Желательно не переопределять методы equals и hashCode сущностей с помощью него;
...
- Vlad Mihalcea описал практически идеальную реализацию, которой чуть-чуть не хватило до оптимального варианта, но ее можно смело использовать;
...
Но он и не нужен, когда есть поисковая строка, которая при вводе запроса выдает:
...
6) [Очень интересная реализация, однако сложна в понимании и реализации](https://jivimberg.io/blog/2018/11/05/using-uuid-on-spring-data-jpa-entities/)
